...
* Prerequisites
** Windows  
   Install Cask, add to path temporarily
   Install pdf-tools with dependencies on MSYS2 then copy epdfinfo to /mingw64/bin/
   Add /mingw64/bin/ to $PATH
   Follow pdf-tools install instructions

   To make ispell work on windows, download and install aspell with any
   language pack then add aspell.exe to your path
   ...
   (let* ((package--builtins nil)
          (packages
           '(auto-compile         ; automatically compile Emacs Lisp libraries
             ...
             cmake-mode           ; Mode for editing CMakeLists.txt files
...
** Visual
   ...
   Change the color-theme to =darktooth=.

   #+BEGIN_SRC emacs-lisp
   (load-theme 'darktooth t)
   #+END_SRC

   =darktooth= is my preferred light theme, but =monokai= makes a very nice
   ...
   (cond ((member "PragmataPro" (font-family-list))
          (set-face-attribute 'default nil :font "PragmataPro-10.5"))
